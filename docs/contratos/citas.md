# Contrato REST — Búsqueda y reserva de citas (HU-012, HU-013, HU-014)

- **Base URL (local):** `http://localhost:8081` (`API_PORT`, D-010)
- **Versionado:** todos los endpoints bajo `/api/v1/` (D-018).
- **Autorización:** `/api/v1/availability` y `/api/v1/appointments/**` exigen rol `USER`. Sin token → `401`; con otro rol (ADMIN, PROFESSIONAL) → `403`.
...
- **Implementación:** `infrastructure/adapters/in/web/booking/`.
...
| 403 | `FORBIDDEN` | Autenticado sin rol `USER` |
